...
void LSM303DLHC_AccFilterConfig(LSM303DLHCAcc_FilterConfigTypeDef * LSM303DLHC_FilterStruct)
{
    ...
    tmpreg |= (uint8_t) (LSM303DLHC_FilterStruct->HighPassFilter_Mode_Selection |
			 ...
			 LSM303DLHC_FilterStruct->HighPassFilter_AOI1 | LSM303DLHC_FilterStruct->HighPassFilter_AOI2);
...
/**
  ...
  */
uint16_t LSM303DLHC_Read(uint8_t DeviceAddr, uint8_t RegisterAddr, uint16_t NumByteToRead, uint8_t * pBuffer)
{
    __IO uint32_t LSM303DLHC_Timeout = LSM303DLHC_LONG_TIMEOUT;
    __IO uint32_t temp;
 ...
 */
uint16_t LSM303DLHC_Write(uint8_t DeviceAddr, uint8_t RegisterAddr, uint16_t NumByteToWrite, uint8_t * pBuffer)
{
...
void LSM303DLHC_I2C_Config(void)
{
    ...
    RCC_APB2PeriphClockCmd(LSM303DLHC_I2C_SCK_GPIO_CLK | RCC_APB2Periph_AFIO, ENABLE);
    RCC_APB2PeriphClockCmd(LSM303DLHC_I2C_SDA_GPIO_CLK | RCC_APB2Periph_AFIO, ENABLE);
...
static void LSM303DLHC_LowLevel_Init(void)
{
    ...
    /* Enable the I2C periph  на APB1 */
    RCC_APB1PeriphClockCmd(LSM303DLHC_I2C_CLK, ENABLE);

    /* Как альтернативные функции Enable SCK and SDA GPIO clocks на APB2  */
    RCC_APB2PeriphClockCmd(LSM303DLHC_I2C_SCK_GPIO_CLK | LSM303DLHC_I2C_SDA_GPIO_CLK | RCC_APB2Periph_AFIO, ENABLE);

    /* Enable DRDY clock на  APB2 */
    RCC_APB2PeriphClockCmd(LSM303DLHC_DRDY_GPIO_CLK | RCC_APB2Periph_AFIO, ENABLE);
