...
#define SD_DETECT_PIN                    GPIO_Pin_3	/* PB.3 */
...
SD_Error SD_Init(void)
{
  ...
  /*----------------- Read CSD/CID MSD registers ------------------*/
    ...
    if (errorstatus == SD_OK) {
///vvv    errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_4b);
...
static SD_Error FindSCR(uint16_t rca, uint32_t * pscr)
{
    ...
    *(pscr + 1) =
	((tempscr[0] & SD_0TO7BITS) << 24) | ((tempscr[0] & SD_8TO15BITS) << 8) | ((tempscr[0] & SD_16TO23BITS) >> 8) | ((tempscr[0] & SD_24TO31BITS) >> 24);

    *(pscr) =
	((tempscr[1] & SD_0TO7BITS) << 24) | ((tempscr[1] & SD_8TO15BITS) << 8) | ((tempscr[1] & SD_16TO23BITS) >> 8) | ((tempscr[1] & SD_24TO31BITS) >> 24);
...
void SD_LowLevel_DeInit(void)
{
    ...
    /*!< Configure PC.08, PC.12 pin: D0, CLK pin */
